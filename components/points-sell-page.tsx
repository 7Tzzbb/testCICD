...
import {ArrowLeft, ShoppingBag, AlertTriangle, CreditCard, Smartphone} from "lucide-react"
...
export default function AssetSellPage() {
    ...
    // 计算实际扣减数量
    const calculateActualDeduction = (amount: string) => {
        if (!amount) return 0
        const numAmount = parseFloat(amount)
        return numAmount * (1 + asset.sellRate)
    }

    // 计算应收总金额
    const calculateTotalReceive = () => {
        if (!sellPrice || !sellAmount) return 0
    ...
    const handleConfirm = async () => {
        if (!sellPrice || parseFloat(sellPrice) <= 0) {
            toast({
                title: t('enterValidSellPrice'),
                description: t('sellPriceGreaterThan0'),
                variant: "destructive",
                duration: 1500
            })
            return
        ...
        if (!sellAmount || parseFloat(sellAmount) < asset.minSellQuantity) {
            toast({
                title: t('enterValidSellQuantity'),
                description: t('sellQuantityGreaterThanOrEqual') + asset.minSellQuantity,
                variant: "destructive",
                duration: 1500
            })
            return
        ...
        if (canSplit && (!minTradeAmount || parseFloat(minTradeAmount) < 1)) {
            toast({
                title: t('enterValidMinSellQuantity'),
                description: t('minSellQuantityGreaterThanOrEqual1'),
                variant: "destructive",
                duration: 1500
            })
            return
        ...
        if (res.code == 0) {
            ...
            toast({
                title: t('sellPostSuccess'),
                description: `${t('sellPosted')}${sellAmount}${asset?.assetsName}，${t('price')}${sellPrice}`,
                duration: 1500
            })
            ...
            if (res.message == 'error.least_deal_amount_not_reached') {
                toast({
                    description: t('error.least_deal_amount_not_reached') + ',' + t('minSellQuantity') + asset?.minSellQuantity,
                    duration: 1500
                })
            } else {
                toast({
    ...
    // 获取可卖数量（从资产数量中提取数值部分）
    const getAvailableAmount = (asset: Asset | null) => {
        if (!asset) return 0
        const amountStr = asset.amount.split(" ")[0]
            ...
            <div
                ...
                <header
                    ...
                    <div className="container flex items-center h-14 px-4">
                        ...
                        <h1 className="font-bold text-lg">{t('sellAssetSection')}</h1>
                    </div>
                </header>
    ...
    return (
        <div
            className="flex flex-col min-h-screen bg-gradient-to-br from-red-500/5 via-background to-red-500/5 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900 pb-16">
            <header
                className="sticky top-0 z-10 backdrop-blur-md bg-white/80 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700">
                <div className="container flex items-center h-14 px-4">
                    <Button variant="ghost" size="icon" onClick={toBack}>
                ...
                >
                  ...
                  <span className="bg-red-500/20 text-red-500 p-1 rounded-md mr-2">
                    <ShoppingBag className="h-4 w-4"/>
                                ...
                                {/* 可卖数量 */}
                                <div className="mb-4 p-3 bg-red-500/5 rounded-lg">
                                    <div
                                        className="text-sm text-gray-600 dark:text-gray-400">{t('availableSellQuantity')}</div>
                                ...
                                {/* 输入价格 */}
                                <div className="mb-4">
                                    <label htmlFor="sell-price" className="block text-sm font-medium mb-1">
                                        {t('enterUnitPrice')}
                                    </label>
                                    <Input
                                        type='number'
                                        id="sell-price"
                                        placeholder={t('pleaseEnterSellPrice')}
                                        value={sellPrice}
                                    ...
                                    <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                        ...
                                        <div className="mt-1">
                                            <span>{t('lowestAsk')}: {asset.priceFloor}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-xs text-gray-500 dark:text-gray-400">{t('minSellQuantity')}{asset.minSellQuantity}</span>
                                    </div>
                                    <Input
                                        type='number'
                                        id="sell-amount"
                                    ...
                                    <div className="mt-1 text-xs text-gray-500 dark:text-gray-400 space-y-1">
                                        <div className="flex justify-between">
                                            <span>{t('feeRate')}: {asset.sellRate * 100}%</span>
                                            <span>{t('actualDeductQuantity')}: {calculateActualDeduction(sellAmount).toFixed(2)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4 p-3 bg-red-500/5 rounded-lg">
                                    <div
                                        className="text-sm text-gray-600 dark:text-gray-400">{t('totalReceivableAmount')}</div>
                                    <div className="text-xl font-bold">
                                        {calculateTotalReceive().toLocaleString()}
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                    ...
                    >
                        <Card
                            className="w-full mb-4 shadow-md hover:shadow-lg transition-all duration-300 bg-gradient-to-r from-background to-red-500/10 vibrant-card hover-glow">
                            <CardContent className="p-4">
                                {/* 可拆分选项 */}
                                <div className="mb-4">
                                ...
                                {canSplit && (
                                    <div className="mb-4 ml-6">
                                        ...
                                        <Input
                                            type='number'
                                            id="min-trade-amount"
                                            placeholder={t('minTransactionQuantity')}
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex items-center space-x-2 rounded-md p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                                            <Checkbox
                                                id="payment-bank"
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div>
                                        ...
                                        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                            <li>{t('sellNoticeBuyerMayAccept')}</li>
                                            <li>{t('sellNoticeFundsReceivedAndAsset')}</li>
                                            <li>{t('sellNoticeServiceFee')}{asset.sellRate * 100}%{t('feeSuffix')}</li>
                    ...
                    >
                        <Button
                            variant="outline"
                            className="flex items-center justify-center gap-2 h-12 hover:bg-red-500/10 dark:hover:bg-gray-800 transition-colors hover-scale"
                            onClick={handleCancel}
                            disabled={isSubmitting}
                        >
                            {t('cancel')}
                        </Button>
                        {/* disabled={isSubmitting || !sellPrice || !sellAmount || parseFloat(sellAmount) < 1} */}
                        <Button
                            variant="default"
                            className="flex items-center justify-center gap-2 h-12 bg-red-500 hover:bg-red-600 transition-colors hover-scale"
                            onClick={handleConfirm}
                        >
                            {isSubmitting ? t('processing') : t('postSell')}
                        </Button>
                    </motion.div>
                </motion.div>
            </main>
