    ...
    useEffect(() => {
        ...
        const connectAndSign = async () => {
            ...
            try {
                ...
                // 获取缓存中的地址信息
                const cachedInfo = getWalletTokenInfo(address);
                const cachedAddress = cachedInfo ? Object.keys(cachedInfo)[0] : null;
                const walletPublicKeys = localStorage.getItem('walletPublicKey')
                // 仅当地址变化且未处理时执行跳转和注册逻辑
                if (address !== currentAddressRef.current && !isAddressChangedRef.current && address !== walletPublicKeys) {
                    console.log("检测到新地址:", address, "与缓存地址不同:", walletPublicKeys);
                    ...
                    console.log("地址未变化，跳过跳转:", address, "缓存地址:", cachedAddress);
                    // 更新当前地址和链 ID，但不跳转
                    setAddress(address);
                    setChainId(chainId);
                    currentAddressRef.current = address;
