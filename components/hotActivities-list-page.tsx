...
export default function HotActivitiesListPage() {
    const [hotActivitiesList, setHotActivitiesList] = useState([]);
    ...
    const fetchMoreData = async (customPage: number) => {
        ...
        const res = await getActivityList(currentPage, pageSize,language)
        if (res.code == 0) {
            const newData = res.data.content
            const totalPages = res.data.totalPages
            // 追加数据
            setHotActivitiesList((prev) =>
        ...
        <div className="flex flex-col min-h-screen bg-gray-100 dark:bg-gray-950">
            <header className="sticky top-0 z-10 bg-white dark:bg-gray-800 shadow-sm h-16 flex items-center">
                <div className="container flex items-center px-4">
                    ...
                    <h1 className="font-bold text-xl text-gray-900 dark:text-white">{t('hotActivities')}</h1>
                </div>
            </header>

            <InfiniteScroll
                dataLength={hotActivitiesList.length}
                next={fetchMoreData}
                hasMore={hasMore}
                loader={<div className="text-center py-4">{t('loading')}</div>}
                endMessage={<div className="text-center py-4">{t('noHotActivities')}</div>}>
                {hotActivitiesList.map((item) => (
                    <div key={item.id}
